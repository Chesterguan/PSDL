...
audit:
  intent: "利用肌酐趋势进行早期AKI检测"
  rationale: "早期AKI检测可实现及时干预"
  provenance: "KDIGO AKI指南（2012年）"

signals:
  Cr:
    ref: creatinine          # 语义引用（通过 Dataset Spec 解析）
    unit: mg/dL
...
ML模型输出通过Dataset Spec绑定作为常规信号处理：
- 模型输出绑定到信号（例如 `sepsis_risk_score`）
- 通过Dataset Spec配置实现模型注册表集成
- 符合PSDL"定义什么，而非如何"的理念
