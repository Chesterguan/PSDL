...
audit:
  intent: "クレアチニントレンドを用いた早期AKI検出"
  rationale: "早期AKI検出により適時な介入が可能"
  provenance: "KDIGO AKIガイドライン（2012年）"

signals:
  Cr:
    ref: creatinine          # セマンティック参照（Dataset Spec で解決）
    unit: mg/dL
...
trends:
  ...
  cr_delta:
    expr: delta(Cr, 6h)
    description: "6時間でのクレアチニン変化量"

  cr_current:
    expr: last(Cr)
    description: "現在のクレアチニン値"

logic:
  # v0.3: 比較はロジック層で 'when' を使用
  cr_rising:
    when: cr_delta > 0.3
    description: "クレアチニン上昇 >0.3 mg/dL"
